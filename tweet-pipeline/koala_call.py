...
def get_label_and_adres(tweet_text):
    ...
    #         GIYSI: Clothes, Blankets, etc.
    #         BARINMA: Shelter
    #         LOJISTIK: Transportation
    #         KURTARMA: Rescue from rubble
    #         SAGLIK: Health, Medicine
    #         YAGMALAMA: Theft
    #         TUVALET: Toilet
    #         ALAKASIZ: Irrelevant
    #     Note that the labels are not mutually exclusive, a tweet can have multiple labels. Lastly, parse the tweet to extract the address information and
    #     save it as a dictionary with the key 'address' and the value as the address as 'adres': 'address information'. If the address is not
    #     found in the tweet, then the value should be an empty string.
    #     Example:
    #         Input: #depremadres #deprem  Göçükten ses gelmediği için vinçler geri gitmis arkadaşlarımız enkaz altında lütfen artık çıkarılsınlar lütfen  ZENGİNLER MAHALLESİ KIRK ASIRLIK TÜRK YURDU CADDESİ (2.YILDIZ APARTMANI) BİNA NO :26 KAPI NO:8 KAT:4 ANTAKYA/ HATAY https://t.co/81JbSYUMEw
    ...
    prompt = """You are AI, a large language model trained by OpenAI. You answer as precisely as possible for each response.
...
User: #depremadres #deprem  Göçükten ses gelmediği için vinçler geri gitmis arkadaşlarımız enkaz altında lütfen artık çıkarılsınlar lütfen  ZENGİNLER MAHALLESİ KIRK ASIRLIK TÜRK YURDU CADDESİ (2.YILDIZ APARTMANI) BİNA NO :26 KAPI NO:8 KAT:4 ANTAKYA/ HATAY https://t.co/81JbSYUMEw
AI: ['KURTARMA'], {'adres': 'Zenginler Mahallesi Kırk Asırlık Tğrk Yurdu Caddesı (2. Yıldız Apartmanı) Bina No:26 Kapı No:8 Kat:4 Antakya/Hatay'}
User: """ + tweet_text + "\n AI: "
